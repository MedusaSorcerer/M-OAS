...
layui.define(["layer"], function (exprots) {
    ...
    var okUtils = {
         ...
         */
        ajax: function (url, type, params, load, headers = true) {
            ...
            $.ajax({
                ...
                data: params || {},
                dataType: "json",
                headers: headers ? {'Authorization': okUtils.local('M&OAS-token')} : {},
                traditional: true,
                beforeSend: function () {
                    if (load) {
                ...
                success: function (data) {
                    if (data.code === 0) {
                        var fullname = window.parent.document.getElementById("fullname");
                        if (fullname) {
                            fullname.innerText = okUtils.local('M&OAS-fullname');
                        }
                ...
                error: function (data) {
                    ...
                    console.log(data)
                    if (data.status === 401) {
                        okUtils.local('M&OAS-token', '');
                        window.top.location.href = "../login.html";
                    } else if (data.status === 404) {
                        layer.msg('访问的资源不存在', {icon: 2, time: 5000});
                        deferred.reject("访问的资源不存在");
                    } else if (error.responseJSON !== undefined) {
                        layer.msg(error.responseJSON.detail, {icon: 2, time: 5000});
                        deferred.reject("okUtils.ajax error: " + error.responseJSON.detail);
                    } else {
                        layer.msg('服务器异常', {icon: 2, time: 5000});
